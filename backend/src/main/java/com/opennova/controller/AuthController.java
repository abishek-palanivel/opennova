...
public class AuthController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest loginRequest) {
        try {
            ...
            if (authService.isAccountLocked(email)) {
                System.err.println("Account is locked: " + email);
                Map<String, String> error = new HashMap<>();
                error.put("message", "Account is locked due to multiple failed login attempts. Please try again after 24 hours.");
            ...
            if (existingUser == null) {
                ...
                authService.handleFailedLogin(email); // Track failed attempt even for non-existent users
                Map<String, String> error = new HashMap<>();
    ...
    public ResponseEntity<?> checkAccountStatus(@RequestBody Map<String, String> request) {
        try {
            ...
            email = email.trim();
            boolean isLocked = authService.isAccountLocked(email);
            
            Map<String, Object> response = new HashMap<>();
            response.put("isLocked", isLocked);
            
            if (isLocked) {
                LocalDateTime lockExpiry = authService.getAccountLockExpiry(email);
                if (lockExpiry != null) {
                    response.put("lockedUntil", lockExpiry.toString());
                    response.put("message", "Account is locked due to multiple failed login attempts. Please try again after 24 hours.");
                }
            }
