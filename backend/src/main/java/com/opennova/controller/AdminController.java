...
public class AdminController {
    ...
    public ResponseEntity<?> getAnalyticsOverview() {
        try {
            ...
            analytics.put("totalEstablishments", totalEstablishments);
            ...
            analytics.put("userGrowth", 12.5); // percentage
            analytics.put("bookingGrowth", 8.3);
            analytics.put("revenueGrowth", 15.7);
            
            return ResponseEntity.ok(analytics);
        } catch (Exception e) {
            Map<String, String> error = new HashMap<>();
            error.put("message", "Failed to fetch analytics overview: " + e.getMessage());
            return ResponseEntity.status(500).body(error);
        }
    }
    ...
    public ResponseEntity<?> getUserGrowthData() {
        try {
            ...
            // Mock data for user growth over last 12 months
            List<Map<String, Object>> growthData = new ArrayList<>();
            String[] months = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
            int[] users = {10, 15, 25, 35, 50, 65, 80, 95, 110, 125, 140, 160};
            
            for (int i = 0; i < months.length; i++) {
                Map<String, Object> dataPoint = new HashMap<>();
                dataPoint.put("month", months[i]);
                dataPoint.put("users", users[i]);
                dataPoint.put("newUsers", i > 0 ? users[i] - users[i-1] : users[i]);
            ...
            Map<String, String> error = new HashMap<>();
            error.put("message", "Failed to fetch user growth data: " + e.getMessage());
            return ResponseEntity.status(500).body(error);
        }
    }
    ...
    public ResponseEntity<?> getBookingTrends() {
        try {
            ...
            int[] bookings = {45, 52, 38, 61, 55, 67};
            
            for (int i = 0; i < weeks.length; i++) {
                Map<String, Object> dataPoint = new HashMap<>();
                dataPoint.put("period", weeks[i]);
                dataPoint.put("bookings", bookings[i]);
                dataPoint.put("revenue", bookings[i] * 150); // Mock revenue calculation
            ...
            Map<String, String> error = new HashMap<>();
            error.put("message", "Failed to fetch booking trends: " + e.getMessage());
            return ResponseEntity.status(500).body(error);
        }
    }
    ...
    public ResponseEntity<?> getEstablishmentDistribution() {
        try {
            ...
            // Get actual establishment distribution by type
            List<Map<String, Object>> distribution = new ArrayList<>();
            
            try {
                // Try to get real data from database
                Map<String, Long> typeCount = establishmentService.getEstablishmentCountByType();
                
                for (Map.Entry<String, Long> entry : typeCount.entrySet()) {
                    Map<String, Object> typeData = new HashMap<>();
                    typeData.put("type", entry.getKey());
                    typeData.put("count", entry.getValue());
                    typeData.put("percentage", 0); // Calculate percentage later
                    distribution.add(typeData);
                }
                
                // Calculate percentages
                long total = distribution.stream().mapToLong(d -> (Long) d.get("count")).sum();
                if (total > 0) {
                    for (Map<String, Object> typeData : distribution) {
                        long count = (Long) typeData.get("count");
                        double percentage = (count * 100.0) / total;
                        typeData.put("percentage", Math.round(percentage * 10.0) / 10.0);
                    }
                }
            } catch (Exception e) {
                // Fallback to mock data
                System.err.println("Using mock data for establishment distribution: " + e.getMessage());
                distribution.clear();
                
                Map<String, Object> hotels = new HashMap<>();
                hotels.put("type", "HOTEL");
                hotels.put("count", 8L);
                hotels.put("percentage", 53.3);
                distribution.add(hotels);
                
                Map<String, Object> hospitals = new HashMap<>();
                hospitals.put("type", "HOSPITAL");
                hospitals.put("count", 4L);
                hospitals.put("percentage", 26.7);
                distribution.add(hospitals);
                
                Map<String, Object> shops = new HashMap<>();
                shops.put("type", "SHOP");
                shops.put("count", 3L);
                shops.put("percentage", 20.0);
                distribution.add(shops);
            }
            
            return ResponseEntity.ok(distribution);
        } catch (Exception e) {
            Map<String, String> error = new HashMap<>();
            error.put("message", "Failed to fetch establishment distribution: " + e.getMessage());
            return ResponseEntity.status(500).body(error);
        }
    }
    ...
    public ResponseEntity<?> getRevenueOverview() {
        try {
            ...
            revenueData.put("totalRevenue", 125000);
            revenueData.put("monthlyRevenue", 15000);
            revenueData.put("averageBookingValue", 150);
            revenueData.put("revenueGrowth", 12.5);
            
            // Monthly revenue breakdown
            List<Map<String, Object>> monthlyBreakdown = new ArrayList<>();
            String[] months = {"Jan", "Feb", "Mar", "Apr", "May", "Jun"};
            int[] revenues = {8000, 9500, 11000, 12500, 14000, 15000};
            ...
            revenueData.put("monthlyBreakdown", monthlyBreakdown);
            
            return ResponseEntity.ok(revenueData);
        } catch (Exception e) {
            Map<String, String> error = new HashMap<>();
            error.put("message", "Failed to fetch revenue overview: " + e.getMessage());
            return ResponseEntity.status(500).body(error);
        }
    }
